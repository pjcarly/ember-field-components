...
export default class InputDateBootstrapComponent extends InputDateComponent {
  ...
  didInsertElement(){
    ...
    options.format = options.hasOwnProperty('format') ? options.format : this.momentFormat;
    options.icons = options.hasOwnProperty('icons') ? options.icons : { previous: 'fa fa-chevron-left', next: 'fa fa-chevron-right' };
    // options.debug = true;
  ...
  get minDate() {
    return this.options && this.options.minDate ? this.options.minDate : undefined;
  }

  @computed('options.maxDate')
  get maxDate() {
    return this.options && this.options.maxDate ? this.options.maxDate : undefined;
  }
