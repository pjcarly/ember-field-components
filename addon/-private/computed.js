    ...
    computedDecorator = function(fn, params, name, importDesc) {
        let dec = decorator(desc => {
            ...
            desc.finisher = function initializeComputedProperty(target) {
                ...
                assert(
                    `ES6 property getters/setters only need to be decorated once, '${key}' was decorated on both the getter and the setter`, !computedDescriptorFor(prototype, key)
                );
...
export function computedDecoratorWithRequiredParams(fn, name, desc) {
    return function(...params) {
        assert(
            `The @${name || fn.name} decorator requires parameters`, !isFieldDescriptor(params) && params.length > 0
        );
